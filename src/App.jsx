...
const styles = {
  main: { 
    backgroundColor: '#faf6f0', minHeight: '100vh', color: '#2c1f0e', 
  ...
  navbar: { 
    ...
    padding: '18px 6%', maxWidth: '1280px', margin: '0 auto', gap: '20px', boxSizing: 'border-box'
  },
  logoWrapper: { 
    display: 'flex', alignItems: 'center', cursor: 'pointer', textDecoration: 'none',
    outline: 'none', border: 'none', WebkitTapHighlightColor: 'transparent', flexShrink: 0
  },
  logoText: { fontSize: '24px', fontWeight: '800', letterSpacing: '-0.5px', color: '#8a5e1e', userSelect: 'none' },
  navRightContainer: {
    display: 'flex', alignItems: 'center', gap: '4px', 
    listStyle: 'none', padding: 0, margin: 0, flexWrap: 'nowrap'
  },
  linkButton: { 
    color: '#6b4f2e', background: 'transparent', backgroundColor: 'transparent',
    border: '1px solid transparent', fontSize: '14px', fontWeight: '500', 
    transition: 'all 0.2s ease', padding: '8px 18px', position: 'relative', 
    letterSpacing: '0.2px', cursor: 'pointer', fontFamily: "'Source Serif 4', serif",
    outline: 'none', borderRadius: '6px', boxShadow: 'none',
    WebkitTapHighlightColor: 'transparent', whiteSpace: 'nowrap'
  },
  activeLink: { 
    color: '#8a5e1e', fontWeight: '700',
    backgroundColor: 'rgba(176,125,58,0.09)', border: '1px solid rgba(176,125,58,0.22)', 
    boxShadow: 'none', borderBottom: '2.5px solid #8a5e1e'
  },
  auditBtn: { 
    padding: '9px 22px', background: '#8a5e1e', border: 'none', borderRadius: '6px', 
    color: '#fdf8f0', fontWeight: '600', cursor: 'pointer', fontSize: '13px',
    letterSpacing: '0.3px', boxShadow: '0 2px 8px rgba(138,94,30,0.25)', 
    transition: 'all 0.2s ease', fontFamily: "'Source Serif 4', serif",
    whiteSpace: 'nowrap', outline: 'none', WebkitTapHighlightColor: 'transparent',
  ...
  glassCard: {
    backgroundColor: '#fff9f2', border: '1px solid rgba(176,125,58,0.12)', borderRadius: '12px', 
  ...
  metricCard: {
    backgroundColor: '#fff9f2', border: '1px solid rgba(176,125,58,0.10)',
    borderRadius: '10px', padding: '28px 24px', transition: 'box-shadow 0.2s ease', 
  ...
  toast: {
    ...
    borderLeft: '3px solid #8a5e1e', border: '1px solid rgba(176,125,58,0.18)',
    borderLeftWidth: '3px', borderLeftColor: '#8a5e1e',
  ...
  adminInput: {
    ...
    marginBottom: '16px', fontFamily: 'sans-serif', boxSizing: 'border-box'
  },
...
const responsiveStyles = `
  ...
  * { box-sizing: border-box !important; }
  ...
  .nav-custom-btn:hover { color: #8a5e1e !important; background-color: rgba(138,94,30,0.07) !important; border: 1px solid rgba(138,94,30,0.15) !important; }
  .login-btn-glow:hover { background: #6e4a17 !important; box-shadow: 0 4px 18px rgba(138,94,30,0.35) !important; transform: translateY(-1px) !important; }
  .service-card:hover { box-shadow: 0 8px 36px rgba(44,31,14,0.10) !important; border-color: rgba(138,94,30,0.20) !important; transform: translateY(-2px) !important; }
  .metric-card-hover:hover { box-shadow: 0 6px 24px rgba(44,31,14,0.09) !important; border-color: rgba(138,94,30,0.18) !important; }
  ...
  .si-dot {
    ...
    background: rgba(176,125,58,0.28);
    border: 1.5px solid rgba(176,125,58,0.20);
    transition: all 0.3s ease; flex-shrink: 0;
  }
  .si-item.active .si-label { opacity: 1; transform: translateX(0); color: #8a5e1e !important; }
  .si-item.active .si-dot {
    width: 13px; height: 13px;
    background: #8a5e1e;
    border: 2px solid rgba(138,94,30,0.25);
    box-shadow: 0 0 0 4px rgba(138,94,30,0.10);
  }
  .si-line {
  ...
  .hero-title  { animation: heroFadeUp 0.75s ease both; animation-delay: 0.15s; }
  .hero-sub    { animation: heroFadeUp 0.75s ease both; animation-delay: 0.35s; }
  .hero-cta    { animation: heroFadeUp 0.75s ease both; animation-delay: 0.55s; }
  ...
  .scroll-arrow {
    ...
    color: #b89a6a; font-size: 11px; font-weight: 600; letter-spacing: 1.2px;
    text-transform: uppercase; cursor: pointer; margin-top: 52px;
    animation: scrollBounce 2s ease-in-out infinite;
    border: none; background: transparent; font-family: 'Source Serif 4', serif;
...
function ScrollIndicator({ activeSection, onDotClick }) {
  return (
    <div className="si-wrap">
      {SECTIONS.map((sec, idx) => (
        <div key={sec.id} className="si-row">
          <div
            className={`si-item${activeSection === sec.id ? ' active' : ''}`}
            onClick={() => onDotClick(sec.id)}
          >
            <span className="si-label" style={{ color: '#b89a6a' }}>{sec.label}</span>
            <div className="si-dot" />
          </div>
          {idx < SECTIONS.length - 1 && <div className="si-line" />}
        </div>
      ))}
    </div>
  );
...
function App() {
  ...
  useEffect(() => {
    ...
    fontNode.innerHTML = `@import url('https://fonts.googleapis.com/css2?family=Source+Serif+4:ital,opsz,wght@0,8..60,200..900;1,8..60,200..900&display=swap'); ${responsiveStyles}`;
  ...
  useEffect(() => {
    ...
    const calculateTime = () => {
      ...
      if (remainingTime <= 0) {
        setLockUntil(0); setFailedAttempts(0);
        localStorage.removeItem('admin_lock_timestamp');
        localStorage.setItem('admin_failed_attempts', '0');
        setTimeLeft(0); triggerToast("Lockout expired. Security panel open.");
  ...
  const handleAuthVerify = (e) => {
    e.preventDefault();
    if (Date.now() < lockUntil) { triggerToast('Console locked.'); return; }
    if (passcode === 'bhai163') {
      setIsAuthorized(true); setFailedAttempts(0);
      localStorage.setItem('admin_failed_attempts', '0');
  ...
  return (
    <div style={styles.main}>
      ...
      {/* ── SCROLL INDICATOR ── */}
      <ScrollIndicator activeSection={activeSection} onDotClick={scrollToSection} />
        ...
        <div style={styles.adminOverlay}>
          <div style={styles.adminModal}>
            <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center'}}>
              ...
              <button onClick={() => { setIsAdminOpen(false); setIsAuthorized(false); setPasscode(''); }}
                style={{background: 'none', border: 'none', color: '#9a7a50', fontSize: '18px', cursor: 'pointer'}}>✕</button>
            </div>
            ...
            {Date.now() < lockUntil ? (
              <div style={{textAlign: 'center', padding: '20px 0'}}>
                ...
                <p style={{color: '#7a6048', fontSize: '14px', marginTop: '6px', lineHeight: '1.5'}}>Too many incorrect authentication parameters. Cooldown active.</p>
              ...
              <form onSubmit={handleAuthVerify} autoComplete="off">
                ...
                <input type="password" value={passcode} onChange={(e) => setPasscode(e.target.value)}
                  style={styles.adminInput} placeholder="••••••••" autoComplete="new-password" />
                <div style={{fontSize: '11px', color: '#9a7a50', marginBottom: '12px'}}>Remaining Entry Node Tokens: {3 - failedAttempts}/3</div>
                <button type="submit" style={{...styles.auditBtn, width: '100%', marginLeft: 0}}>Verify Dashboard</button>
              </form>
            ) : (
              <div style={{display: 'flex', flexDirection: 'column'}}>
                ...
                <button onClick={() => { setIsAdminOpen(false); setIsAuthorized(false); setPasscode(''); triggerToast("Changes deployed successfully!"); }}
                  style={{...styles.auditBtn, background: 'linear-gradient(135deg, #2d8a55 0%, #1a6b3c 100%)', marginTop: '15px', marginBottom: '10px', marginLeft: 0}}>
                  Save & Live Deploy
                </button>
              </div>
            )}
      ...
      <section id="home" style={styles.scrollSection} ref={homeRef}>
        <div style={styles.viewWrapper}>
          <div style={styles.heroSection}>
            ...
            <p className="hero-sub" style={styles.subtitleText}>{liveData.homeSubtitle}</p>
            <button
              className="scroll-arrow hero-cta"
              onClick={() => scrollToNode(servicesRef)}
        ...
        <div style={styles.viewWrapper}>
          ...
          <div style={styles.gridContainer}>
            <div style={{...styles.glassCard, transition: 'all 0.25s ease'}} className="service-card">
              <div style={{marginBottom: '20px'}}><span style={{fontSize: '10px', color: '#7c5cbf', fontWeight: '700', border: '1px solid rgba(124,92,191,0.2)', padding: '3px 10px', borderRadius: '4px', letterSpacing: '1px'}}>ORGANIC MANAGEMENT</span></div>
              ...
              <div style={{borderTop: '1px solid rgba(176,125,58,0.10)', paddingTop: '20px'}}>
                <ul style={{listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '14px', color: '#5a4030'}}>
                  {['Daily Instagram Profile Strategy','Professional Facebook Page Tuning','High Engagement Creative Analytics'].map(t => (
                    <li key={t} style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
                      <span style={{width:'18px',height:'18px',borderRadius:'50%',backgroundColor:'rgba(124,92,191,0.12)',display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:'10px',color:'#7c5cbf',flexShrink:0}}>✓</span>{t}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div style={{...styles.glassCard, transition: 'all 0.25s ease'}} className="service-card">
              <div style={{marginBottom: '20px'}}><span style={{fontSize: '10px', color: '#8a5e1e', fontWeight: '700', border: '1px solid rgba(138,94,30,0.22)', padding: '3px 10px', borderRadius: '4px', letterSpacing: '1px'}}>PAID CHANNELS</span></div>
              ...
              <div style={{borderTop: '1px solid rgba(176,125,58,0.10)', paddingTop: '20px'}}>
                <ul style={{listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '14px', color: '#5a4030'}}>
                  {['Targeted Google Search Campaigns','High Converting Meta Video Lead Ads','Daily Cost Management & Scaling Optimization'].map(t => (
                    <li key={t} style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
                      <span style={{width:'18px',height:'18px',borderRadius:'50%',backgroundColor:'rgba(138,94,30,0.10)',display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:'10px',color:'#8a5e1e',flexShrink:0}}>✓</span>{t}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div style={{...styles.glassCard, transition: 'all 0.25s ease'}} className="service-card">
              <div style={{marginBottom: '20px'}}><span style={{fontSize: '10px', color: '#2d8a55', fontWeight: '700', border: '1px solid rgba(45,138,85,0.2)', padding: '3px 10px', borderRadius: '4px', letterSpacing: '1px'}}>WEB ARCHITECTURE</span></div>
              ...
              <div style={{borderTop: '1px solid rgba(176,125,58,0.10)', paddingTop: '20px'}}>
                <ul style={{listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '14px', color: '#5a4030'}}>
                  {['Smooth Interactive User Interfaces','Lightning Fast Page Speed & Loading Time','All Mobile Responsive Clean Design Layouts'].map(t => (
                    <li key={t} style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
                      <span style={{width:'18px',height:'18px',borderRadius:'50%',backgroundColor:'rgba(45,138,85,0.10)',display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:'10px',color:'#2d8a55',flexShrink:0}}>✓</span>{t}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div style={styles.viewWrapper}>
          ...
          <div style={styles.glassCard}>
            <div style={{display: 'flex', flexDirection: 'column', gap: '26px'}}>
              ...
              <div style={styles.gridContainer}>
                {[
                  {title:'Social Media Management', desc:'Sahi strategy aur rules ke saath pages ko manage aur daily brand reputation grow karna.'},
                  {title:'Search Engine Optimization (SEO)', desc:'Google search result ranking ko improve karke free customer traffic laana.'},
                  {title:'High-End Website Development', desc:'Tez chalne wali modern custom single page websites jo business ka trust build karein.'},
                  {title:'Targeted Google & Meta Ads', desc:'Sahi logon ko low investment cost par real-time products aur business ads show karna.'},
                ].map(card => (
                  <div key={card.title} style={styles.metricCard} className="metric-card-hover">
                    <h4 style={{fontSize:'16px',fontWeight:'600',color:'#1a1008',marginBottom:'8px',letterSpacing:'-0.2px'}}>{card.title}</h4>
                    <p style={{color:'#8a6a45',fontSize:'14px',lineHeight:'1.6',margin:0}}>{card.desc}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <footer className="site-footer">
        © {new Date().getFullYear()} <strong>DigiGrow</strong> · Digital Marketing Agency · Prayagraj, India
      </footer>
